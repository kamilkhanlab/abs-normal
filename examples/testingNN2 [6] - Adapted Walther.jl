...
function generate_RELU(
    ...
    if account_for_time == true 
        CCOEFF[1] = THETA[1][:, end] * timeval + CCOEFF[1]
...
function calculate_ODE(
    ...
    for i in initialize:occurences
        ...
        #TODO:
        Z_coeff, L_coeff, c_coeff, J_coeff, Y_coeff, b_coeff = generate_RELU(numLayers, THETA, CCOEFF, account_for_time = true, timeval=time[i])
...
Z_coeff, L_coeff, c_coeff, J_coeff, Y_coeff, b_coeff = generate_RELU(numLayers, THETA, CCOEFF, account_for_time = false)
xk_output = calculate_ODE(
    n, xk_raw_data, initalize, dt, occurences, 
    0, 0, 0, 0, 0, 0,
    # Z_coeff, L_coeff, c_coeff, J_coeff, Y_coeff, b_coeff,

...
function generate_RELU(numLayers, n, THETA, CCOEFF)
    ...
    ZETA[rowIdx[end], colIdx[end]] = THETA[end]
...
#function set up M()
    ...
    META = zeros(numRow, numCol)
    META[rowIdx[1], colIdx[1]] = I(n[1])
    for i in 2:numLayers
        META[rowIdx[i], colIdx[i]] = I(n[i])
        META[rowIdx[i], colIdx[i-1]] = -0.5 .* THETA[i-1] 
    end #for 

    LETA = zeros(numRow, numCol)
    for i in 2:numLayers
        LETA[rowIdx[i], colIdx[i-1]] = 0.5 .* THETA[i-1] 
    end #for 
    LETA = hcat(ones(numRow, n[1]), LETA)
    ZETAnew = META\LETA 

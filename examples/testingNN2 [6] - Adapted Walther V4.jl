...
function generate_RELU(
    ...
    n = [size.(THETA, 2); size(THETA[end], 1)]
    n[1] = n[end]
    n_input = n[1]; n_output = n[end]; n_array = n[2:end-1]; n_swap = n_input + 1
    ...
    colIdx = vcat(0, cumsum([n_swap; n_array]))
    ...
    #TODO: ADJUST TO ACCOUNT FOR NEW SYSTEM:
    META = META[1:(n_neurons+n_input), 1:(n_neurons+n_input)]
    ZETA = ZETA[1:(n_neurons+n_input), :]

    #Set up a coefficient matrix β (BETA)
    BETA_base = cat(CCOEFF..., dims=1)
    BETA_base = META \ BETA_base

    BETA_time = ZETA[:, n_input]
    # ZETA = [ZETA[:, 1:n_input] ZETA[:, n_input+2:end]]
    ZETA = [ZETA[:, 1:n_input] ZETA[:, n_input+2:end]]
    @show ZETA[1, 4]
...
function generate_RELU_ODE(
    ...
    #Calculate time intermediate value
    time_intermediate = THETA[1][:, end] 
    THETA[1] = THETA[1][:, 1:end-1]

    #Calculate neuron array information
    n = [size.(THETA, 2); size(THETA[end], 1)]
    n_total = sum(n[2:end]); n_neurons = sum(n[2:end-1])
    n_input = n[end]; 
    ...
    #Set up a coefficient matrix ζ (ZETA) 
    ZETA = META \ intermediate
    display(ZETA) #FUNCTIONAL - DOUBLE CHECKED WITH MATLAB 
    ...
    return Z_coeff, L_coeff, J_coeff, Y_coeff, BETA_base, time_intermediate, META
end #function 
...
function calculate_ODE(
    ...
    Z_coeff, L_coeff, J_coeff, Y_coeff, BETA_base, time_intermediate, META;
    account_for_time = false
)
    n[1] = n[end]
    n_input = n[1]; n_output = n[end];
    n_swap = n_output
    n_neurons = sum(n[2:end-1])
    ...
    for i in initialize:occurences
        ...
        BETA_total = BETA_change(BETA_base, n[2], time_intermediate, timeI[i], META) #TODO
        # @show BETA_total 
        ...
        Z_coeff_I, L_coeff_I, c_coeff_I, J_coeff_I, Y_coeff_I, b_coeff_I = adjust_RELU_ODE(
            Z_coeff, L_coeff, c_coeff_I, J_coeff, Y_coeff, b_coeff_I, P, Q, r
        )
...
Z_coeff1, L_coeff1, J_coeff1, Y_coeff1, BETA_base1, time_intermediate1, META1 = generate_RELU_ODE(n, numLayers, THETA, CCOEFF)
# Z_coeff2, L_coeff2, J_coeff2, Y_coeff2, BETA_base2, BETA_time2 = generate_RELU(numLayers, THETA, CCOEFF)

xk_output = calculate_ODE(
    n, xk_raw_data, initialize, dt, occurences, 
    Z_coeff1, L_coeff1, J_coeff1, Y_coeff1, BETA_base1, time_intermediate1, META1;

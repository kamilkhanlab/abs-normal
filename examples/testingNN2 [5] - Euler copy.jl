...
Z=29
xk_raw_data, xk_absnorm, xk_fwd_pass, num_outputs = calculate_it(Z, 0.0101, 100)
plot_it(xk_raw_data, xk_absnorm, xk_fwd_pass, num_outputs)

##(7) Re-run for Walther Formulation:
Z=29; occurrences=100; dt=0.0101;
# xf_CCOEFF = XLSX.readxlsx(string("data/DataSet", Z, "/FitRNet_Storage_CCOEFF.xlsx"))
# xf_THETA = XLSX.readxlsx(string("data/DataSet", Z,"/FitRNet_Storage_THETA.xlsx"))
# xk_raw_data = XLSX.readxlsx(string("data/DataSet", Z,"/FitRNet_Storage_RawData.xlsx"))[1][:]'
...
rowIdx = vcat(0, cumsum(n[2:end]))
rowIdx = [rowIdx[i]+1:rowIdx[i+1] for i in 1:numLayers]
colIdx = vcat(0, cumsum(n[1:end-1]))
colIdx = [colIdx[i]+1:colIdx[i+1] for i in 1:numLayers]

inter = zeros(sum(n[2:end]), sum(n[1:end-1]))
for i in 1:numLayers - 1
    inter[rowIdx[i+1], rowIdx[i]] = 0.5 .* THETA[i+1]
end #for 
META = I(sum(n[1:end-1]))[1:end-1, :] - inter

inter = [zeros(sum(n[2:end]), n[1]) inter[:, 1:end-n[1]]]
inter[rowIdx[1],colIdx[1]] = THETA[1]
fastZETA = round.(pinv(META) * inter, digits=3)

CETA = reduce(vcat, CCOEFF)
fastCETA = round.(pinv(META) * CETA, digits=3)

#Now:
Z_coeff = fastZETA[1:sum(n[2:end-1]),  colIdx[1]]
J_coeff = fastZETA[rowIdx[end],        colIdx[1]]

L_coeff = LowerTriangular(fastZETA[1:sum(n[2:end-1]),  n[1]+1:sum(n[1:end-1])])
Y_coeff = fastZETA[rowIdx[end],                        n[1]+1:sum(n[1:end-1])]

c_coeff = fastCETA[1:sum(n[2:end-1])]
b_coeff = fastCETA[rowIdx[end]]

#Test zero adjustment:
b_coeff = [b_coeff; 0]
Y_coeff = [Y_coeff; zeros(1, 39)]
J_coeff = [J_coeff; zeros(1, 5)]

#Set up initial condition:
num_outputs, occurences, initialize = 4, 100, 2
xk = zeros(num_outputs, occurences+1)
time = [dt * i for i in 0:occurences]
xk[:, 1:initialize] = xk_raw_data[:, 1:initialize]
...
# J_coeff_ = I(n[1])[1:end-1,:] - dt .* J_coeff
J_coeff_ = I(n[1]) - dt * I(n[1]) * J_coeff
Y_coeff_ = -dt * I(n[1]) * Y_coeff
b_coeff_ = -[xk_raw_data[:, 1]; time[1]] - dt * I(n[1]) * b_coeff

#Finally | Test 1:
anfI = AbsNormal.AnfCoeffs(c_coeff_, b_coeff_, Z_coeff_, L_coeff_, J_coeff_, Y_coeff_)  
#Calculate root via LCP:
rootLCP, terminationStatusLCP = AbsNormal.solve_pa_equation(anfI, approach=AbsNormal.BY_LCP);
if any(isnan.(rootLCP))
    println(terminationStatusLCP)
...
for i in initialize:occurences
    ...
    # J_coeff_I = I(n[1])[1:end-1,:] - dt .* J_coeff
    J_coeff_I = I(n[1]) - dt * I(n[1]) * J_coeff
    Y_coeff_I = -dt * I(n[1]) * Y_coeff
    b_coeff_I = -[xk_raw_data[:, i]; time[i]] - dt * I(n[1]) * b_coeff

    anfI = AbsNormal.AnfCoeffs(c_coeff_I, b_coeff_I, Z_coeff_I, L_coeff_I, J_coeff_I, Y_coeff_I) 
    #Calculate root via LCP:
    try
        rootLCP, terminationStatusLCP = AbsNormal.solve_pa_equation(anfI, approach=AbsNormal.BY_LCP);
        xk[:, i+1] = rootLCP[1:num_outputs]
        if any(isnan.(rootLCP))
            println(terminationStatusLCP)
        end 
    catch err
...
plot_it(xk_raw_data, xk, 0, num_outputs)
...
for i in initialize:occurences
    ...
    # J_coeff_I = I(n[1])[1:end-1,:] - dt .* J_coeff
    J_coeff_I = I(n[1]) - dt .* J_coeff
    Y_coeff_I = -dt .* Y_coeff
    b_coeff_I = -[xk[:, i]; tupdate] - dt .* b_coeff
...
#=
    ...
    META = I(len) - inter
    inter = [zeros(len,n[1]) inter[:, 1:(len-n[1])]]
    inter[rowIdx[1],colIdx[1]] = THETA[1]
    CETA = reduce(vcat, CCOEFF)
